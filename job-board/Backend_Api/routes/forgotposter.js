            ...
            Employer.findOne({ email })
                .then(user => {
                    bcrypt.hash(password, 10)
                        .then(hashedPassword => {
                            Employer.updateOne({ email: user.email }, { password: hashedPassword }, function(err, data) {
                                if (err) throw err;
...
router.post("/registrationEmail", (req, res) => {
    ...
    try {
        ...
        const mailOptions = {
            ...
            to: email,
            subject: "JobHive",
            html: `<h3> Hii ${name}, </h3><br><p>Successfully Register</p><br><p>Congratulation now u r member of the JobHive</p>`,
            text: 'Thank You.... !'
        }
