...
const PostLogin = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await fetch("http://localhost:5000/api/employer/postlogin", {
      method: 'POST',
      headers: {
    ...
    if (json.success) {
      ...
      localStorage.setItem('isEmployer', json.isEmployer);
      navigate('/postjob');
    }
  ...
  return (
    <div className="login-design">
      ...
      <div className="login-image">
        <img src={login} alt="chiku" className='logo-photo'/>
      </div>
      ...
      <div className="login-form">
        ...
        <div className="forgot-signup">
          ...
          <p className="signup-para">Don't have an account ? <NavLink className="signup-redirect" to="/postsignup">Create Now !</NavLink></p>
        </div>
      </div>
    </div>
  );
